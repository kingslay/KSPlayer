...
class SeekView: UIView {
    private var seekToViewImage = UIImageView()
    private var seekToLabel = UILabel()
...
extension SeekView: SeekViewProtocol {
    public func set(text: String, isAdd: Bool) {
        ...
        seekToViewImage.transform = CGAffineTransform(rotationAngle: isAdd ? 0 : CGFloat(Double.pi))
    }
}
